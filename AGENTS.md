...
- On the first concrete task in a new chat/session, before task-specific work,
  ...
  when the local update contract allows it, and report a compact status that
  includes the pending migration count, including `0` when none are pending.
  Report a compact blocker if the check cannot complete.
...
- On Windows, never send non-ASCII JSON or form text through PowerShell
  ...
  request body is explicitly encoded as UTF-8 bytes and the content type includes
  `charset=utf-8`. Prefer a Node script/fetch for API writes that contain
  Russian or other non-ASCII text. After any admin/API content write, read the
  saved value back through the API and fail the check if it contains `????` or
  the replacement character `�`. For mojibake, inspect whole fragments such as
  `Рџ`, `Рљ`, `СЃ`, or `С‚` in places where readable Russian is expected; do not
  treat a single normal Cyrillic letter such as `Р` or `С` as corruption.
